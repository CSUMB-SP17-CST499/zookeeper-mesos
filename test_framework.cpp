...
class MesosZk : public Scheduler {
public:
    MesosZk(bool _implicitAcknowledgements, const string& _role)
        ...
        const vector<Offer>& offers)
    {
        ...
        while (NODES > 0) {
            foreach (const Offer& offer, offers) {
                cout << "Received offer " << offer.id() << " with " << offer.resources()
                     << " on " << offer.hostname() << endl;
                Resources taskResources = Resources::parse("cpus:" + stringify(CPUS_PER_TASK) + ";mem:" + stringify(MEM_PER_TASK))
                                              .get();
                taskResources.allocate(role);
                Resources offerResource = offer.resources();
                if (offerResource.flatten().contains(taskResources)) {
                    if (zk_offers.count(offer.hostname()) == 0 && NODES > 0) {
                        zk_offers[offer.hostname()] = offer;
                        NODES--;
                        if (NUM_MASTERS > 0) {
                            NUM_MASTERS--;
        ...
        launch_tasks(zk_offers, masters, agents, driver);
    }
    // Lauches task per offer
    void launch_tasks(map<string, Offer> zk_offers, vector<string> masters,
        vector<string> agents, SchedulerDriver* driver)
    {
        for (auto const& offer : zk_offers) {
            int taskId = tasksLaunched++;
            cout << "\nLaunching task " << taskId << " using offer "
                 << offer.second.id() << endl;
            std::string command = "";
            // if offer is for a master
            generate_conf(masters);
            if (std::find(masters.begin(), masters.end(), offer.second.hostname()) != masters.end()) {
                command = generate_master(agents, masters, offer.second.hostname());

            }

            // if offer is for agent
            else {
                command = generate_agent(masters, offer.second.hostname());
            }
            vector<TaskInfo> tasks;
            TaskInfo task;
            task.set_name("Task " + lexical_cast<string>(taskId));
            task.mutable_task_id()->set_value(lexical_cast<string>(taskId));
            task.mutable_slave_id()->MergeFrom(offer.second.slave_id());
            ...
            Resources offerResource = offer.second.resources();
            Option<Resources> resources = offerResource.find(flattened.get());
            task.mutable_resources()->MergeFrom(resources.get());

            tasks.push_back(task);
            driver->launchTasks(offer.second.id(), tasks);
        }
    }

    // generating master command
    std::string generate_master(vector<string> agents, vector<string> masters,
        string self)
    {
        std::ifstream f("setup.sh");
        std::string setup((std::istreambuf_iterator<char>(f)),
        ...
        setup.replace(setup.find("%t"), string("%t").length(),
            string(create_command + echo_command));
    ...
    std::string generate_conf(vector<string> masters)
    {
        ...
        std::string conf((std::istreambuf_iterator<char>(f)),
            std::istreambuf_iterator<char>());
    ...
    virtual void statusUpdate(SchedulerDriver* driver, const TaskStatus& status)
    {
        ...
        if (status.state() == TASK_LOST || status.state() == TASK_KILLED || status.state() == TASK_FAILED || status.state() == TASK_ERROR) {
...
int main(int argc, char** argv)
{
    ...
    MesosZk scheduler(implicitAcknowledgements, flags.role);
    ...
    if (os::getenv("MESOS_AUTHENTICATE_FRAMEWORKS").isSome()) {
        ...
        driver = new MesosSchedulerDriver(&scheduler, framework, flags.master.get(),
